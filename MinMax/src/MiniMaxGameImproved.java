...
public class MiniMaxGameImproved {
	...
	public static void main(String[] args)
	{
		...
		{
			System.out.println("Part 4:MiniMaxGameImproved board3.txt board4.txt 2");
			System.exit(0);
		}
		...
		else if(!args[1].equals("C:\\Users\\Rassu\\Desktop\\board5E4.txt"))	// if the name of the output file is not board4.txt
		{
			System.out.println("Part 4: MiniMaxGameImproved board3.txt board4.txt 2");
			System.exit(0);
		}
		depth=Integer.parseInt(args[2]);
		MiniMaxGameImproved mmgi = new MiniMaxGameImproved();
		mmgi.generateMidgameEndgame(args[0],args[1]);
		Link parent=L.first;
		int minimax_estimate=mmgi.maxMin(parent,0);	
		...
		System.out.println("Position evaluated by static estimation is: " + pos_evaluated);	// prints the number of positions evaluated
		System.out.println("MiniMax estimate is: " + minimax_estimate);								// prints the minimax estimate value
    ...
    private int minMax(Link temp, int d)
    {
    	...
    	else
    	{
    		...
    		while(extra!=null)
    		{
    			temp_maxMin=maxMin(extra,d);	//v=min(v,maxMin(y));
    			if(temp_maxMin<v)
    				v=temp_maxMin;
    			extra=extra.nextLink;
    		}
	...
	int static_estimation(String b)
	{
		...
		else return(1000*(num_WhitePieces-2*num_BlackPieces)-numBlackMoves);
